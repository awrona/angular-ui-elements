...
const ngModuleName = 'cre8.core.util.util';
...
.factory('$cre8Util', function($cacheFactory, $document, $timeout, $q, $window, $cre8Constant) {
  var Util;
  ...
  return Util = {
    ...
    transitionEndPromise: function(element, opts) {
      ...
      element.on($cre8Constant.CSS.TRANSITIONEND, finished);
      function finished(ev) {
        // Make sure this transitionend didn't bubble up from a child
        if (!ev || ev.target === element[0]) {
          element.off($cre8Constant.CSS.TRANSITIONEND, finished);
          deferred.resolve();
        }
      }
